...
#for single==================================
    def getOrderHistoryBitfinex(self,accountId,dateValue,wallet=Wallet()):
        nonce = dateValue
        api_key = wallet.account.api_key
        api_secret_key = bytes(wallet.account.api_secret_key)
        url = '/v1/orders'
        completeUrl = ExchangeTypeUrl.BITFINEX.value + url
        ...
        if data:
            try:
                ...
                orderList = Order.query.filter_by(account_id=accountId,order_type = OrderTypeEnum.SINGLE.value).order_by(Order.date_time).all()
                json_object = json.loads(data.content)
                OrderValueTypeList=[]


                for orderObject in orderList:
                    orderPaymentDetail=OrderPaymentDetail.query.filter_by(order_id=orderObject.id,order_type=OrderTypeEnum.SINGLE.value).first()
                    if orderPaymentDetail.uuid is not None:
                        updateOrderPaymentDetail(orderPaymentDetail)
                orderList = Order.query.filter_by(account_id=accountId, order_type=OrderTypeEnum.SINGLE.value).order_by(
                    Order.date_time).all()
